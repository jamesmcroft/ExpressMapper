...
namespace ExpressMapper
{
    ...
    public class DestinationTypeMapper<T, TN> : TypeMapperBase<T, TN>, ITypeMapper<T, TN>
    {
        ...
        public DestinationTypeMapper(IMappingService service, IMappingServiceProvider serviceProvider)
        ...
        {
            var mapMethod = typeof(IMappingServiceProvider).GetTypeInfo().DeclaredMethods
        ...
        protected override void CompileInternal()
        {
            ...
            if (this.BeforeMapHandler != null)
            {
                ...
                var beforeInvokeExpr = Expression.Invoke(
                    beforeExpression,
                    this.SourceParameter,
                    this.DestFakeParameter);
            ...
            var customProps = this.CustomPropertyCache.Where(k => !this.IgnoreMemberList.Contains(k.Key))
                .Select(k => k.Value);
            ...
            var expression = Expression.Lambda<Func<T, TN, TN>>(
                resultExpression,
                this.SourceParameter,
                this.DestFakeParameter);
