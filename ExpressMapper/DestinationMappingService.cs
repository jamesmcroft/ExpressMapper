...
namespace ExpressMapper
{
    ...
    internal class DestinationMappingService : MappingServiceBase, IMappingService
    {
        ...
        public override BlockExpression MapCollection(
            Type srcColtype,
            Type destColType,
            ...
            Expression destExpression)
        {
            ...
            var notNullCondition = Expression.IfThenElse(
                conditionToCreateList,
                this.MapCollectionNotCountEquals(
                    srcExpression.Type,
                    destExpression.Type,
                    srcExpression,
                    destExpression),
            ...
            var result = Expression.IfThenElse(
                Expression.NotEqual(destExpression, StaticExpressions.NullConstant),
                notNullCondition,
        ...
        public override BlockExpression MapProperty(
            Type srcType,
            Type destType,
            Expression srcExpression,
            Expression destExpression,
            bool newDest)
        {
            ...
            var ifDestNull = destType.GetTypeInfo().IsPrimitive || destType.GetTypeInfo().IsEnum
                                 ? (Expression)StaticExpressions.FalseConstant
                                 : Expression.Equal(destExpression, StaticExpressions.NullConstant);
            ...
            var substBlock = new PreciseSubstituteParameterVisitor(
                                 new KeyValuePair<ParameterExpression, ParameterExpression>(
                                     mapExpressions.Item2,
                                     sourceVariable),
                                 new KeyValuePair<ParameterExpression, ParameterExpression>(
                                     mapExpressions.Item3,
        ...
        public override void PrecompileCollection<T, TN>()
        {
            ...
            var result = Expression.IfThenElse(
                Expression.NotEqual(destVariable, StaticExpressions.NullConstant),
                notNullCondition,
                newCollBlockExp);
            ...
            var checkSrcForNullExp = Expression.IfThenElse(
                Expression.Equal(sourceVariable, StaticExpressions.NullConstant),
                Expression.Default(destVariable.Type),
                resultExpression);
            ...
            Expression destVariable)
        {
            ...
            var isICollection = !destVariable.Type.IsArray
                                && (destVariable.Type.GetTypeInfo().ImplementedInterfaces.FirstOrDefault(
                                        t => t.GetTypeInfo().IsGenericType
                                             && t.GetGenericTypeDefinition() == typeof(ICollection<>)) != null
            ...
            if (isICollection)
            {
                ...
                var removeCollFirstExp = Expression.Call(
                    destVariable,
                    destCollection.GetTypeInfo().GetDeclaredMethod("Remove"),
                    getFirstEnumExp);

                var brkColRem = Expression.Label();
                var loopToDropColElements = Expression.Loop(
                    Expression.IfThenElse(
                        Expression.GreaterThan(destCount, srcCount),
                        removeCollFirstExp,
                ...
                var mapCollectionSourcePrevail = this.MapCollectionSourcePrevail(
                    destVariable,
                    sourceType,
                    sourceVariable,
                    destType);
                var collBlock = Expression.IfThenElse(
                    Expression.GreaterThan(destCount, srcCount),
                    collRemoveBlockExp,
            ...
            else
            {
                ...
                var enumeratorSrc = Expression.Variable(
                    closedEnumeratorSourceType,
                    $"{Guid.NewGuid().ToString("N")}EnumSrc");
                var assignToEnumSrc = Expression.Assign(
                    enumeratorSrc,
                    Expression.Call(sourceVariable, closedEnumerableSourceType.GetTypeInfo().GetDeclaredMethod("GetEnumerator")));
                var doMoveNextSrc = Expression.Call(enumeratorSrc, typeof(IEnumerator).GetTypeInfo().GetDeclaredMethod("MoveNext"));
                ...
                var enumeratorDest = Expression.Variable(closedEnumeratorDestType, $"{Guid.NewGuid():N}EnumDst");
                var assignToEnumDest = Expression.Assign(
                    enumeratorDest,
                    Expression.Call(destVariable, closedEnumerableDestType.GetTypeInfo().GetDeclaredMethod("GetEnumerator")));
                var doMoveNextDest = Expression.Call(
                    enumeratorDest,
                ...
                var innerLoopBlock = Expression.Block(
                    new[] { srcItmVarExp, destItmVarExp },
        ...
        private BlockExpression MapCollectionCountEquals(
            Type tCol,
            Type tnCol,
            Expression sourceVariable,
            Expression destVariable)
        {
            ...
            var closedEnumeratorDestType = typeof(IEnumerator<>).MakeGenericType(destType);
            var closedEnumerableDestType = GenericEnumerableType.MakeGenericType(destType);
            var enumeratorDest = Expression.Variable(
                closedEnumeratorDestType,
                $"{Guid.NewGuid().ToString("N")}EnumDst");
            var assignToEnumDest = Expression.Assign(
                enumeratorDest,
                Expression.Call(destVariable, closedEnumerableDestType.GetTypeInfo().GetDeclaredMethod("GetEnumerator")));
            var doMoveNextDest = Expression.Call(enumeratorDest, typeof(IEnumerator).GetTypeInfo().GetDeclaredMethod("MoveNext"));
            var currentDest = Expression.Property(enumeratorDest, "Current");
            var destItmVarExp = Expression.Variable(destType, $"{Guid.NewGuid().ToString("N")}ItmDst");
            var assignDestItmFromProp = Expression.Assign(destItmVarExp, currentDest);
            ...
            var expressions = new List<Expression> { assignToEnumSrc, assignToEnumDest, loopExpression };

            var blockExpression = Expression.Block(parameters, expressions);
            return blockExpression;
        }

        private BlockExpression MapCollectionSourcePrevail(
            Expression destVariable,
            Type sourceType,
            Expression sourceVariable,
            Type destType)
        {
            ...
            var addToNewCollNew = Expression.Call(
                destVariable,
                destCollection.GetTypeInfo().GetDeclaredMethod("Add"),
                destItmVarExp);

            var ifFalseBlock = Expression.Block(
                new ParameterExpression[] { },
                new[] { mapExprForType, addToNewCollNew });
            ...
            var ifNotEndOfListExp = Expression.IfThen(
                Expression.Equal(endOfListExp, StaticExpressions.FalseConstant),
